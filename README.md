...
## AI Captioning
To use the AI captioning feature, you must have the [VLM Caption Server](https://github.com/sergey-skatov/vlm_caption_server) running locally. The app is configured to use the AI captioning service at `http://localhost:8000/` by default. You can change this URL in the [Preferences](#preferences-configuration) dialog. The AI captioning feature is disabled by default. To enable it, check the "Enable AI Captioning" checkbox in the Preferences dialog. You must also [load a model and select a prompt](#ai-captions-dropdown-lists) (above the table).
